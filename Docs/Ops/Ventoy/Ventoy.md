...
在制作 Bootable USB drive 时，通常需要将 U 盘格式化后，然后用写盘工具写入镜像。如果不做其他操作默认会写入整个 USB，这样即浪费空间管理也不方便。Ventoy 就是一个用于解决这个问题的开源工具
...
ventoy 和 微PE 一样可以将需要的文件注入到 live OS 中，但是 ventoy 是通过脚本的方式注入
...
```
{
  ...
  "theme": {
    "default_file": "0",
    "file": [
      "/ventoy/theme/wannacry/theme.txt",
      "/ventoy/theme/reaper/theme.txt",
      "/ventoy/theme/redskull/theme.txt"
    ],
    "gfxmode": "1920x1080",
    "display_mode": "GUI",
    "ventoy_left": "2%",
